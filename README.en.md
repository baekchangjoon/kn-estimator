...
ℹ No project-specific calibration — estimated with the bundled (LegacySut) coefficients …
```
...
it was measured on a single project (LegacySut), so **absolute USD is not
...
src/kn_estimator/
  ...
  data/calibration.json  # bundled calibration (18 LegacySut runs)
...
15 tests depend on the original SUT (LegacySut) or an external sample and are
