...
- **적용 가능**: 문서 페이지 번역, 클래스 단위 리팩토링, 파일 단위 마이그레이션,
  ...
  단위별 목록 생성 레시피와 개념 설명은 `docs/CONCEPTS.md`.
- `group`이 컨트롤러 친화 묶음을 대체하고, 캘리브레이션·청크·벽·보고서 로직은
  수정 없이 동작한다. 캘리브레이션도 동일 — 그 작업 유형(라벨)으로 파일럿
  2점을 실측하면 된다 (계수는 작업 유형·하네스마다 다르므로 재사용 불가).
- **단위 일관성 계약**: 파일럿 원장의 n과 추정 목록의 N은 **같은 단위**로
  세어져야 한다 — 도구가 검증할 수 없는 유일한 사용자 책임이다. 라벨 이름에
  단위를 담아두면(예: `analyze-class`) 실수를 줄인다.
