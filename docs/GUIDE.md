...
실측 43 run(LegacySut, `results/run_ledger.jsonl`)에서 확인된 사실:
...
캘리브레이션은 특정 프로젝트(LegacySut) 실측이므로, 대상 프로젝트의 엔드포인트가
...
1. **매트릭스에서 구성을 고른다** — 예상 총비용·벽시계로. 실측 근거상 기본 권장은
   `template×sonnet`, 비용 절대 최소가 필요하면 `template×haiku`(단, 검증 깊이
   캐비앗 — final-report §4).
...
- Java/Spring + Maven/Gradle 구조 가정. 다른 스택은 스캐너 교체 필요
  (`scan.py`의 inventory/슬라이스 규칙만 바꾸면 모델·플랜은 재사용 가능).
