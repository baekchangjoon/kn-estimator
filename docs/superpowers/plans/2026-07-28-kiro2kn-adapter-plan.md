...
**D9 (2026-07-28 후속, 사용자 UX 피드백) — 한 줄 경로.** `--latest`가 현재
디렉토리(또는 `--cwd`)의 가장 최근 대화를 자동 선택해 목록·id 전사 단계를
없애고, `--mode/--model`(kn-estimate와 같은 어휘)로 variant를 내부 조립한다
(`--variant`는 직접 지정용으로 유지, 동시 지정은 오류). 매칭 부재 시 `--list`
안내와 함께 실패.

**D10 (2026-07-28 후속, 사용자 결정) — --label 범용화 + kn-kiro 승격.**
`--mode`(고정 enum)와 원장 `variant`(레거시 6종 명칭)를 폐기하고, 셀을
`<label>/<model>`(label=자유 문자열)로 일반화했다 — 하위호환 없음(정규 배포 전),
캠페인 원장 3종은 label/model 필드로 마이그레이션(셀 동일성 재생성 검증).
단일 N 셀의 flat/opus 기준 차용 폴백도 제거(동봉 셀 어디도 미사용 확인 —
단일 N은 `single_n` 사유로 스킵). 어댑터는 `src/kn_estimator/kiro.py`로 이동해
`kn-kiro` 콘솔 스크립트로 승격(D6의 '엔트리포인트 없음' 결정을 사용자 지시로
번복), `--variant` 옵션 제거. 역사 기록(캠페인 보고서 등)과 기존 run_id
문자열(runs/ 디렉토리명과 결합)은 유지한다.
