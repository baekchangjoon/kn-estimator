...
ℹ 이 프로젝트의 자체 캘리브레이션이 없습니다 — 동봉(LegacySut) 계수로 추정했습니다 …
```
...
동봉 계수는 단일 프로젝트(LegacySut) 실측이라 **절대 USD는 보증하지 않습니다** —
...
src/kn_estimator/
  ...
  data/calibration.json  # 동봉 캘리브레이션 (LegacySut 실측 18 run)
...
SUT(LegacySut)·외부 샘플 의존 테스트 15건은 해당 경로가 없으면 건너뜁니다
