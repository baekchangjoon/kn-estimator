...
캘리브레이션은 패키지에 동봉돼 있어 별도 데이터 없이 바로 동작한다. 단 `--calibration`
없이 실행하면 동봉(LegacySut) 계수 사용 사실과 파일럿 캘리브레이션 절차(크기가 다른
그룹 2개 이상 실측 → `kn-calibrate` → 재실행; 캠페인 설계 N 2점×반복 3 실측에서 오차
−34%→±10%)를 고지한다 — 절대 금액이 필요하면 그 절차를 먼저 밟는 것을 권장한다.
원장 스키마는 [docs/GUIDE.md](docs/GUIDE.md) §4.4에 있다.
...
대상 프로젝트(`legacy-sut/`)는 저장소에 포함되지 않는다. 없으면 해당 테스트는 건너뛴다.
...
- **절대 USD는 보증하지 않는다** — 캘리브레이션이 단일 프로젝트(LegacySut) 실측 기반이다.
